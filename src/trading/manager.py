...
class TradingBotManager:
    ...
    async def _lowbuy_open(
        ...
        # 再次校验入场价在 [MIN_ENTRY, MAX_ENTRY] 区间内 (scan() 已过滤, 但双重保险)
        if ask_price < LOWBUY_MIN_ENTRY or ask_price > LOWBUY_MAX_ENTRY:
            logger.warning(
                "[LowBuy] 入场价 %.2f¢ 超出区间 [%.0f,%.0f]¢, 拒单",
                ask_price * 100, LOWBUY_MIN_ENTRY * 100, LOWBUY_MAX_ENTRY * 100,
            )
            return None
        ...
        for t in state.get("trades", []):
            if (
                ...
                logger.info("[LowBuy] 跳过: %s slug=%s 已有过交易（每窗口一仓）",
                            outcome.get("label"), slug[-14:])
                return None
        ...
        try:
            ...
            if position:
                ...
                position["strategy"] = "lowbuy_double"
