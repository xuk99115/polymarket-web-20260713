...
if TYPE_CHECKING:
    ...
    from .live_trader import LiveTrader as _LiveTrader
...
class PaperExecutor(BaseExecutor):
    ...
    async def close_position(self, position, exit_price, exit_reason, signal=None) -> str:
        ...
        # Older state files may contain an empty or partially populated stats map.
        stats = state.setdefault("stats", {})
        stats.setdefault("total_trades", 0)
        stats.setdefault("winning_trades", 0)
        stats.setdefault("losing_trades", 0)
        stats.setdefault("total_profit", 0.0)
...
class LiveExecutor(BaseExecutor):
    ...
    def _aggregate_fills(self, tracked_ids: set[str], orders: List[Dict[str, Any]]) -> Optional[Dict[str, Dict[str, Any]]]:
        if not tracked_ids:
            return {}
    ...
    async def sync_state(self, markets_by_slug: Optional[Dict[str, Dict[str, Any]]] = None, now_utc: Optional[datetime] = None) -> List[str]:
        ...
        if live_open_orders is None:
            logger.warning("实盘订单同步失败，保留本地订单状态并跳过本轮")
            return ["实盘订单同步失败，本轮未变更订单状态"]
        open_order_ids = {
        ...
        if fills_by_order is None:
            logger.warning("实盘成交同步失败，保留本地订单状态并跳过本轮")
            return ["实盘成交同步失败，本轮未变更订单状态"]
