...
* ability to see price changes *between* snapshots before the market re-prices
...
class _BookState:
    ...
    def apply_snapshot(
        ...
        # Full replacement - one size per price level.
        self.bids = sorted(dict(bids).items(), key=lambda item: -item[0])
        self.asks = sorted(dict(asks).items(), key=lambda item: item[0])
        self.hash = hash_
    ...
    def apply_price_change(self, price: float, size: float, side: str) -> bool:
        if side not in _VALID_SIDES:
            return False
        # Polymarket reports the resting order side: BUY is a bid, SELL is an ask.
        book = self.bids if side == _BUY else self.asks
...
class PolymarketWSClient:
    ...
    def apply_message(self, msg: Any) -> bool:
        ...
        if isinstance(msg, list):
            changed = False
            for item in msg:
                changed = self.apply_message(item) or changed
            return changed
        if not isinstance(msg, dict):
            return False
    ...
    def _handle_price_change(self, msg: Dict[str, Any]) -> bool:
        data = msg.get("price_changes") or msg.get("data") or []
    ...
    async def connect(self, token_ids: List[str]) -> None:
        ...
        if self.is_running:
            if new_ids and self._ws is not None:
                await self._send_subscription_update(new_ids, "subscribe")
            return
    ...
    async def _send_subscription_update(
        ...
        try:
            await self._ws.send(
                json.dumps({"assets_ids": token_ids, "operation": operation})
            )
        except ConnectionClosed:
            logger.debug("WS closed before subscription update could be sent")
