...
import {
    ...
    renderConfig, renderFvSignalHistory, renderTrades,
...
export async function fetchBotStatus() {
    ...
    try {
        ...
        const latestSignal = Array.isArray(data.fv_signals) ? data.fv_signals[0] : null;
        const action = latestSignal ? 'BUY' : 'SKIP';
        const chosenLabel = String(latestSignal?.outcome_label || '').toUpperCase();
        ...
        if (predEl) {
            predEl.textContent = action === 'BUY'
                ? (chosenLabel ? `买 ${chosenLabel}` : 'FV 买入')
                : 'FV 观望';
...
export async function fetchFvSignalHistory() {
    try {
        const resp = await fetch('/api/fv-signals?ts=' + Date.now(), { cache: 'no-store' });
        const data = await resp.json();
        dashboardState.fvSignalHistory = Array.isArray(data) ? data : [];
        renderFvSignalHistory();
    } catch (e) {
        const list = document.getElementById('ai-history-list');
        if (list) list.innerHTML = '<div class="empty-row">FV 信号历史读取失败</div>';
    }
}
window.fetchFvSignalHistory = fetchFvSignalHistory;
...
export async function saveSystemSettings() {
    ...
    const positionVal = parseFloat(_getInputVal('cfg-input-position')) || 2.0;
    const edgeVal = parseFloat(_getInputVal('cfg-input-edge')) || 300;
    const mteVal = parseFloat(_getInputVal('cfg-input-mte')) || 2.0;
    const maxPosVal = parseInt(_getInputVal('cfg-input-max-positions'), 10) || 1;
    const btcAgeVal = parseInt(_getInputVal('cfg-input-btc-age'), 10) || 10;

    // 基础字段总是覆盖
    const updatePayload = {
        TRADING_MODE:       selectedMode,
        FV_EDGE_POSITION_USD: positionVal,
        FV_EDGE_THRESHOLD_BPS: edgeVal,
        FV_EDGE_MAX_MTE: mteVal,
        FV_EDGE_MAX_OPEN_POSITIONS: maxPosVal,
        FV_EDGE_MAX_BTC_AGE_SECONDS: btcAgeVal,
    };
...
export function refreshAll() {
    ...
    Promise.allSettled([
        ...
        fetchTrades(), fetchOrders(), fetchFvSignalHistory(), fetchOrderBook()
    ]);
}
