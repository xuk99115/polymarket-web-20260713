...
class StatusServerSmokeTest(unittest.TestCase):
    ...
    def test_api_config(self):
        ...
        self.assertIsInstance(body, dict)
        # 关键字段都得在
        for k in ("trading_mode", "ai_decision_interval_seconds", "paper_start_balance"):
    ...
    def test_api_ai_decisions(self):
        status, body = self._get("/api/ai-decisions")
        self.assertEqual(status, 200, "/api/ai-decisions 必须 200")
        self.assertIsInstance(body, list)

    def test_api_orderbook(self):
        """orderbook 需要 market slug, 没有配 slug 时会 400, 这是预期。"""
        status, _ = self._get("/api/orderbook")
        # 200 或 400 都可以, 不能 5xx
        self.assertIn(status, (200, 400), f"/api/orderbook 不应 5xx, 实际 {status}")
    ...
    def test_post_update_config_validates_skill_size(self):
        """AI_TRADING_SKILL 长度上限必须生效。"""
        status, body = self._post("/api/update-config", {
            "AI_TRADING_SKILL": "X" * 5000,
        })
        self.assertEqual(status, 400, "skill > 4096 字节应被 400 拦下")
        self.assertIsInstance(body, dict)
        self.assertIn("error", body)

    def test_post_update_config_accepts_small_skill(self):
        status, body = self._post("/api/update-config", {
            "AI_TRADING_SKILL": "short skill",
        })
        self.assertEqual(status, 200, "合理 skill 应通过")
